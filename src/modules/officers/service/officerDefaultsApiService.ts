...
const transformGuardApiToOfficerDefaults = (apiResponse: GuardApiDefaultsResponse): OfficerDefault[] => {
  ...
  return Object.entries(groupedByDate).map(([date, defaults]) => ({
    officerId: checkinLogs[0]?.guardId || uniformChecks[0]?.guardId || "",
    date,
    defaults,
  }));
};
...
const fetchOfficerDefaults = async (
  ...
  if (!response.data.success) {
    throw new Error("Failed to fetch officer defaults");
  }

  console.log(`✅ Successfully fetched officer defaults for: ${officerId}`, response.data.data.summary);

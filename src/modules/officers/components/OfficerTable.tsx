...
const OfficerTable: React.FC<OfficerTableProps> = ({ filters }) => {
  ...
  const handleRowClick = (officer: Officer) => {
    // Navigate to officer performance page
    // Convert the officer name to a URL-friendly format
    const officerNameForUrl = officer.name.replace(/\s+/g, "-").toLowerCase();
    navigate(`/officers/${officerNameForUrl}/performance`);
  };

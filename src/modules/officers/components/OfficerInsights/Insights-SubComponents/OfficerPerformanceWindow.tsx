...
const OfficerPerformanceWindow: React.FC = () => {
  // Get officer name from URL params
  const { officerName } = useParams<{ officerName: string }>();

  // Get officers data from context
  const { getOfficerByName, loading: officersLoading } = useOfficers();
  ...
  // Use TanStack Query for officer defaults with caching and date range
  // Note: Using officerData.id as the officer ID (which equals guard_id)
  const {
    data: allDefaults = [],
  ...
  } = useOfficerDefaults(officerData?.id || null, !!officerData?.id, fromDate, toDate);

  // Use TanStack Query for officer incidents
  const {
    data: incidents = [],
  ...
  } = useOfficerIncidents(officerData?.id || null, !!officerData?.id);

  // Use TanStack Query for officer tasks
  const {
    data: tasks = [],
  ...
  } = useOfficerTasks(officerData?.id || null, !!officerData?.id);
  ...
  // Keep existing officer assignment logic
  useEffect(() => {
    if (!officerData) return;

    setLoadingAssignmentData(true);
    officerPerformanceDataService
      .getOfficerAssignment(officerData.id, selectedDate)
      .then((data) => {
  ...
  const handleRefreshAll = async () => {
    if (officerData?.id) {
      console.log(`🔄 Force refreshing all officer data for: ${officerData.id}`);

      // Invalidate all officer caches
      invalidateOfficerDefaults(officerData.id);
      invalidateOfficerIncidents(officerData.id);
      invalidateOfficerTasks(officerData.id);

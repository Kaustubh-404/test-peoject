...
const transformApiResponseToInternal = (apiResponse: ApiDefaultsResponse): GuardDefault[] => {
  const { checkinLogs, uniformChecks, geofenceLogs, alertnessChecks } = apiResponse.data;

  // Group data by date
  const groupedByDate: Record<string, DefaultData[]> = {};

  // Process checkin logs (LATE defaults)
  checkinLogs.forEach((log) => {
    const date = log.dutyDate.split("T")[0]; // Extract YYYY-MM-DD
    ...
    if (log.status === "LATE") {
      ...
      groupedByDate[date].push({
        ...
        displayText: `${formatTime(log.actualTime)} - ${formatTimeDifference(log.timeDifference)}`,
      });
    }
  });
  ...
  return Object.entries(groupedByDate).map(([date, defaults]) => ({
    guardId:
      checkinLogs[0]?.guardId ||
      uniformChecks[0]?.guardId ||
      geofenceLogs[0]?.guardId ||
      alertnessChecks[0]?.guardId ||
      "",
    date,
    defaults,
  }));
...
const fetchGuardDefaults = async (guardId: string, fromDate?: string, toDate?: string): Promise<GuardDefault[]> => {
  ...
  console.log(`✅ Successfully fetched defaults for guard: ${guardId}`, response.data.data.summary);

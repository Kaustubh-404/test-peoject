...
const AssignmentWindow: React.FC = () => {
  ...
  // Get guard data from context to find the guard ID
  const { getGuardByName, loading: guardLoading } = useGuards();

  // Find the guard data
  const guardData = guardName ? getGuardByName(guardName) : null;
